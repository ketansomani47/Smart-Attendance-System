...
public class StudentPanel extends AppCompatActivity {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(id==R.id.studentChangePassword){
            Intent i=new Intent(StudentPanel.this,StudentVerification.class);
            startActivity(i);

        }
        else if(id==R.id.studentEditProfile){
            Intent i=new Intent(StudentPanel.this,StudentProfile.class);
            startActivity(i);

        }
        else if(id==R.id.studentLogout){
            SharedPreferences pref=getSharedPreferences("myFile",MODE_PRIVATE);
            SharedPreferences.Editor edit=pref.edit();
            edit.putBoolean("isStudentLogin",false);
            edit.commit();
            Intent l=new Intent(StudentPanel.this,StudentLogin.class);
            finish();
            startActivity(l);
        }
        else if(id==R.id.studentNotification){
